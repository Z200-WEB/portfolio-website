...
const translations = {
  ...
  jp: {
    ...
    languages: {
      title: '言語スキル',
      japanese: { name: '日本語', level: 'ビジネスレベル' },
...
const WorkExperienceSection = ({ lang }) => {
  ...
  const isJP = lang === 'jp';
  const experiences = [
    { icon: '📚', titleJP: '英語講師', titleEN: 'English Instructor', typeJP: 'アルバイト', typeEN: 'Part-time', period: '2024年 - 現在', periodEN: '2024 - Present', current: true, descJP: ['小学生を対象に英語指導（語彙・文法・会話）','理解度に応じたレッスンプラン作成','保護者と連携し進捗報告'], descEN: ['English instruction for elementary school students','Creating lesson plans tailored to student comprehension','Collaborating with parents and reporting student progress'], tagsJP: ['教育','コミュニケーション','語学'], tagsEN: ['Education','Communication','Language'], gradient: 'from-blue-600/20 to-cyan-600/20', border: 'border-blue-500/30', iconBg: 'bg-blue-500/20' },
    { icon: '🏪', titleJP: 'セブンイレブン', titleEN: '7-Eleven', typeJP: 'アルバイト', typeEN: 'Part-time', period: '2023年 - 現在', periodEN: '2023 - Present', current: true, descJP: ['レジ業務（正確で迅速な会計）','接客対応で顧客満足度向上','商品補充・店内整理','チームでの店舗運営'], descEN: ['Cash register operations','Customer service to improve satisfaction','Product restocking and store organization','Team-based store operations'], tagsJP: ['接客','チームワーク','コミュニケーション'], tagsEN: ['Customer Service','Teamwork','Communication'], gradient: 'from-orange-600/20 to-amber-600/20', border: 'border-orange-500/30', iconBg: 'bg-orange-500/20' },
    { icon: '🍜', titleJP: 'リンガーハット', titleEN: 'Ringer Hut', typeJP: 'アルバイト', typeEN: 'Part-time', period: '2023年 - 2024年', periodEN: '2023 - 2024', current: false, descJP: ['ホール業務（接客・オーダー）','キッチン補助（調理・盛り付け）','清掃・備品管理','チームワーク重視'], descEN: ['Hall operations (customer service, order taking)','Kitchen assistance (cooking, plating)','Cleaning and equipment management','Emphasis on teamwork'], tagsJP: ['接客','チームワーク','調理補助'], tagsEN: ['Customer Service','Teamwork','Kitchen Assist'], gradient: 'from-red-600/20 to-pink-600/20', border: 'border-red-500/30', iconBg: 'bg-red-500/20' },
  ];
  return (
      ...
      <div className="max-w-6xl mx-auto relative">
        <ScrollReveal>
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-3">{isJP ? 'アルバイト経験' : 'Part-time Experience'}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {experiences.map((exp, i) => (
            <ScrollReveal key={i} delay={i * 150}>
              <div className={`relative rounded-3xl border bg-gradient-to-br ${exp.gradient} ${exp.border} backdrop-blur-xl overflow-hidden cursor-pointer transition-all duration-500 hover:scale-[1.02] hover:-translate-y-1 hover:shadow-[0_0_40px_rgba(139,92,246,0.15)]`} onClick={() => setExpandedCard(expandedCard === i ? null : i)}>
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className={`w-12 h-12 rounded-2xl ${exp.iconBg} flex items-center justify-center text-2xl`}>{exp.icon}</div>
                    {exp.current && (<div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-500/20 border border-green-500/30"><div className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" /><span className="text-xs text-green-400 font-medium">{isJP ? '在職中' : 'Current'}</span></div>)}
                  </div>
                  <div className="mb-3">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-bold text-white">{isJP ? exp.titleJP : exp.titleEN}</h3>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-white/10 text-gray-300">{isJP ? exp.typeJP : exp.typeEN}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-gray-400 text-sm"><span>📅</span><span>{isJP ? exp.period : exp.periodEN}</span></div>
                  </div>
                  <div className="flex flex-wrap gap-1.5 mb-4">{(isJP ? exp.tagsJP : exp.tagsEN).map((tag, t) => (<span key={t} className="text-xs px-2.5 py-1 rounded-full bg-white/[0.08] text-gray-300 border border-white/10">{tag}</span>))}</div>
                  <div className="text-xs text-gray-500">{expandedCard === i ? (isJP ? '閉じる ↑' : 'Close ↑') : (isJP ? '詳細を見る ↓' : 'View details ↓')}</div>
                </div>
                <div className={`overflow-hidden transition-all duration-500 ${expandedCard === i ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <div className="px-6 pb-6 border-t border-white/[0.08] pt-4">
                    <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-3">{isJP ? '仕事内容' : 'Responsibilities'}</p>
                    <ul className="space-y-2">{(isJP ? exp.descJP : exp.descEN).map((item, d) => (<li key={d} className="flex items-start gap-2 text-sm text-gray-300"><span className="text-purple-400 mt-0.5 flex-shrink-0">▸</span>{item}</li>))}</ul>
                  </div>
                </div>
              </div>
            </ScrollReveal>
